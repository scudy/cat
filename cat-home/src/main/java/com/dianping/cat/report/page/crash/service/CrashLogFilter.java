...
public class CrashLogFilter {
	...
	public CrashLogFilter(String query) {
		if (StringUtils.isNotEmpty(query)) {
			...
			if (querys.size() == 5) {
				m_appVersions = Splitters.by(SPLITTER).noEmptyItem().split(querys.get(0));
				m_platformVersions = Splitters.by(SPLITTER).noEmptyItem().split(querys.get(1));
				m_modules = Splitters.by(SPLITTER).noEmptyItem().split(querys.get(2));
				m_levels = Splitters.by(SPLITTER).noEmptyItem().split(querys.get(3));
				m_devices = Splitters.by(SPLITTER).noEmptyItem().split(querys.get(4));
			}
		}
	}
